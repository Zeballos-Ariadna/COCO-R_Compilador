...
public class Scanner {
	...
	static {
		...
		for (int i = 58; i <= 58; ++i) start.set(i, 1);
		for (int i = 65; i <= 90; ++i) start.set(i, 1);
		for (int i = 95; i <= 95; ++i) start.set(i, 1);
		for (int i = 97; i <= 122; ++i) start.set(i, 1);
		for (int i = 0; i <= 33; ++i) start.set(i, 2);
		for (int i = 35; i <= 37; ++i) start.set(i, 2);
		for (int i = 40; i <= 57; ++i) start.set(i, 2);
		for (int i = 59; i <= 59; ++i) start.set(i, 2);
		for (int i = 61; i <= 61; ++i) start.set(i, 2);
		for (int i = 64; i <= 64; ++i) start.set(i, 2);
		for (int i = 91; i <= 94; ++i) start.set(i, 2);
		for (int i = 96; i <= 96; ++i) start.set(i, 2);
		for (int i = 123; i <= 65535; ++i) start.set(i, 2);
		start.set(60, 18); 
		start.set(63, 19); 
		start.set(62, 14); 
		start.set(39, 15); 
		...
		literals.put("=", new Integer(6));
		literals.put("/", new Integer(8));
		...
		loop: for (;;) {
			switch (state) {
				...
				case 1:
					...
					if (ch >= '-' && ch <= '.' || ch >= '0' && ch <= ':' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); state = 1; break;}
					else {t.kind = 1; break loop;}
				case 2:
					recEnd = pos; recKind = 2;
					if (ch <= '!' || ch >= '#' && ch <= '%' || ch >= '(' && ch <= '9' || ch == ';' || ch == '=' || ch >= '?' && ch <= '@' || ch >= '[' && ch <= '^' || ch == '`' || ch >= '{' && ch <= 65535) {AddCh(); state = 2; break;}
				...
				case 8:
					{t.kind = 3; break loop;}
				case 9:
					if (ch == 'x') {AddCh(); state = 10; break;}
					else {state = 0; break;}
				case 10:
					if (ch == 'm') {AddCh(); state = 11; break;}
					else {state = 0; break;}
				case 11:
